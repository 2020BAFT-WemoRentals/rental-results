...
setwd("C:/Users/owuser/Documents/Wemo")
...
for(district in districts) {
  ...
  district.ts <- msts(district.df$rent_nums, seasonal.periods= c(24, 168), start=c(0,1))
  ...
  for(j in seq(from=dst_fixed.nTrain + 1, to=(length(district.ts) - 168), by= 168)) {
    ...
    dst_train.ts <- window(district.ts, start=c(0, 1), end= c(0, j - 1))
    dst_valid.ts <- window(district.ts, start=c(0, j), end= c(0, j - 1 + stepsAhead))
    dst_fit.pred <- snaive(ts(dst_train.ts, freq=24*7), h= stepsAhead)
    set.seed(201)
    dst_train.nnetar <- nnetar(dst_train.ts, repeats=20 )
    dst_fit.pred <- forecast(dst_train.nnetar, h= stepsAhead)
    ...
    main_header <- paste("Neural Network", district.name, "Time Series Chart (", n, "Roll )")
    ...
    dst_validSnaiveE <- dst_valid.ts - dst_fit.pred$mean[169:336]
    dst_trainSnaiveE <- dst_fit.pred$residuals[(j-672):(j-1)]
    ## plot for the whole residuals in district
    trainSnaiveE.df <- data.frame(dst_trainSnaiveE)
    trainSnaiveE.seq <- trainSnaiveE.df$dst_trainSnaiveE
    
    # gap seq with NAs
    empty_seq <- seq(1, 168, 1)
    empty_seq[empty_seq != 0] <- NA
    
    validSnaiveE.df <- data.frame(dst_validSnaiveE)
    validSnaiveE.seq <- validSnaiveE.df$dst_validSnaiveE
    
    wholeSnaiveE.seq <- c(trainSnaiveE.seq, empty_seq, validSnaiveE.seq)
    error_main_header <- paste("Neural Network",district.name, "Error Chart (", n, "Roll )")
    plot(wholeSnaiveE.seq, xlab="Time", ylab="Errors", main=error_main_header, ylim=c(-250, 250), x=full_index[(j-672):(j+335)], type="l", col="red", lty=1, lwd=2)
    abline(h=0, lty=2)
    lines(y=dst_validSnaiveE,x=full_index[(j+168):(j+335)], col="pink", lwd=2)
    
    # accuracy show
    accuracy.ret <- accuracy(dst_fit.pred, dst_valid.ts)
    print(district.name)
    ...
    file.name <- paste("Neural Network", district.name, n, "roll ret.csv")
