...
for(district in districts) {
  ...
  for(j in seq(from=dst_fixed.nTrain + 1, to=(length(district.ts) - 168), by= 168)) {
    ...
    main_header <- paste("Seasonal Naive", district.name, "Time Series Chart (", n, "Roll )")
    ...
    ## plot for the whole residuals in nangang
    ...
    error_main_header <- paste("Seasonal Naive",district.name, "Error Chart (", n, "Roll )")
    ...
    file.name <- paste("Seasonal Naive", district.name, n, "roll ret.csv")
